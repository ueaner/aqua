...
func (dl *Downloader) getReadCloserFromGitHubArchive(ctx context.Context, file *File) (io.ReadCloser, int64, error) {
	if rc, length, err := dl.http.Download(ctx, fmt.Sprintf(config.GitHubArchiveURLTemplate, file.RepoOwner, file.RepoName, file.Version)); err == nil {
		return rc, length, nil
	}
	// e.g. https://github.com/anqiansong/github-compare/archive/3972625c74bf6a5da00beb0e17e30e3e8d0c0950.zip
	if rc, length, err := dl.http.Download(ctx, fmt.Sprintf(config.GitHubArchiveURLTemplate2, file.RepoOwner, file.RepoName, file.Version)); err == nil {
		return rc, length, nil
	}
